...
4. **🔴 CHECKPOINT · 🛑 STOP**: 把产出展示给用户确认:"骨架我理解对了吗?有没有你希望重点突出的方向?" 得到确认再进入阶段 1。
...
**🔴 CHECKPOINT · 🛑 STOP (用户轻确认)** ★: 筛选完成后,把"通过的 N 个候选标题 + 淘汰的 M 个"列表展示给用户:"这 N 个会做成 skill,有想捞回或砍掉的吗?" 得到确认再进入阶段 2 — 阶段 2–4 是最耗时的部分,这一步确认能避免大量返工。
...
| 触发条件 | 一线修复动作 | 仍失败兜底方案 |
| --- | --- | --- |
| 当前环境不支持并行 spawn 5 个 sub-agent | 自动降级为串行调用 5 个 extractor prompt | 改用合并 prompt，一次性提炼所有维度的粗略大纲再让用户确认 |
| 用户提供的 PDF / EPUB 无法解析或乱码 | 提示用户使用文本提取工具转换，或索要纯文本 `TXT` | 暂停任务，等待用户提供可用文本后再从 `PIPELINE_STATE.md` 恢复 |
| 阶段 4 盲测未通过（混淆测试失败） | 返回阶段 2 重写 `SKILL.md`，增加更明确的上下文和防呆设定 | 将失败的具体案例和对比结果提交给用户，请求人工修正方向 |

## 🔴 反例与黑名单 (不要做的事)

1. **不要凭记忆拆书**：在没有拿到完整源文本（或者转写稿）的情况下，绝对禁止通过大模型的预训练记忆来“编造”技能。宁可停下来向用户索要文件。
2. **不要做普通的读后感或总结**：Cangjie 的目标是“可执行的方法论”，如果提取出的内容只是心得体会，必须无情砍掉，绝不写入最终版。
3. **不要静默跑完全程**：必须在阶段 0 和 阶段 1.5 严格等待 `🔴 CHECKPOINT` 确认，严禁一口气输出最终结果，防止方向性偏离导致的巨大计算浪费。
