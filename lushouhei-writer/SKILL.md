...
> - **放行后的三条硬规则**：① 用户没提供的动作、心理、对话、场景、过往经历、情绪自述（如「我以前也……」「我也焦虑过」）一律写成【待补：xxx】，不自己补；② 你引入的外部数据和事实，全部列进文末待补清单请用户核实；③ 【待补】超过 8 处，或核心经历段落整段都是占位 → 改判为 STOP，先要素材再写。正文不低于 4000 字：用户素材不够撑篇幅时，用「AI擅长做的」补足（背景科普、证据佐证、类比、历史文化参照、逐一展示的升番），不用编的经历凑字。
