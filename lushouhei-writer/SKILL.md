...
> 🔴 **CHECKPOINT 1 · 选题对齐（动笔前必停）**
> 在进入写作前，先用一两句话跟用户对齐，等用户确认再继续：
> - HKR判定结果（这个选题占了H/K/R里的哪几项，够不够S级）
> - 你打算切入的核心角度（一句话讲清楚）
> - **如果选题只占一项或素材信息不足 → 🛑 STOP，先问清楚再写，绝不硬着头皮开写。**
...
> 🔴 **CHECKPOINT 2 · 开写前确认三件事（缺一不可）**
> 第二步反复强调「核心创意角度」「第一手真实经历」必须人来提供。所以正式扩写前，先跟用户确认到位，等用户回复再动笔：
> 1. **文章原型**：这篇属于下面五种原型里的哪一种（调查实验/产品体验/现象解读/工具分享/方法论）
> 2. **核心角度**：那个让文章立住的"对，就是这个！"的创意点，由用户拍板
> 3. **真实经历/细节**：用户有没有可放进去的第一手经历或情绪节点
> - **如果第2、3项缺失 → 🛑 STOP，先问用户要，不要用AI编造假经历（这是绝对禁区第5条）。** 用户实在给不出真实经历时，如实写"这块我没真实素材"而非硬编。
...
**如果 选题只占 HKR 里的一项（CHECKPOINT 1 没过）** → 🛑 不进入写作。把缺的那一两项当面跟用户讲清楚（比如「有信息量但不够有趣，开头容易劝退」），一起调角度，调到至少占两项再动笔。

**如果 第一手经历／核心创意角度拿不到（CHECKPOINT 2 第2、3项缺）** → 🛑 不用AI编造经历（绝对禁区第5条）。先问用户要；用户确实给不出，就老实写「这块我没真实素材」或「我自己还没试过，但想想就觉得xxx」，宁可缺，不可假。
